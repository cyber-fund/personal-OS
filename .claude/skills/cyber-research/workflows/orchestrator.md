...
All paths relative to the workspace root (the folder opened in Cowork).

**Company research — if a project folder exists** at `./private/projects/<company-slug>/`:
```
./private/projects/<company-slug>/context/research/MMDD-<slug>-YY/
├── raw/
└── report.md
```
...
```
./content/research/<category>/<slug>/MMDD-<slug>-YY/
├── raw/
└── report.md
```
...
- Company/Market: Investment lens (venture rubric)
...
### 5.2 Update Project Context (Company Only)

If research type is Company and a project folder exists:
- Check if `./private/projects/<company>/README.md` exists
- If yes: optionally append a line to the project's `status.md` noting new research
- If no: suggest the user run `/cyber-create-project "<company>"` to start tracking the deal

### 5.3 Log Completion

Append to `./private/.cybos/logs/MMDD-YY.md`:
